...
private:
   ...
   * This defines the condition (small residual) for a successful completion
   * of KINSOL.
   */
  double ftol;

  /**
   * The Newton method will terminate when the scaled norm of the update is
   * smaller than steptol.
   */
  double steptol;

  /**
   * Maximum number of nonlinear iteration that can be done without a
   * Jacobian update.
   */
  double mbset;
  ...
  /**
   * Scaling flag: indicate if the scaling have been provided by the user
   ...
   * note that the direct solver of KINSOL works only in serial.
